...
<? if (!empty($arResult["ITEMS"])) : ?>
						...
						<div class="item" id="<?= $this->GetEditAreaId($arElement["ID"]); ?>">
							<div class="tabloid">
								...
								<a href="<?= $arElement["DETAIL_PAGE_URL"] ?>" class="picture">
									<span class="picture_inner" style="background-image: url('<?= $picture["src"] ?>')"></span>
	...
	<? if (empty($arParams["AJAX"])) : ?>
		...
		$signParams = $signer->signParameters($this->__component->__name, $arParams);
		?>
		<script type="text/javascript">
			var ajaxDirBrandList = '<?= $this->GetFolder(); ?>';
			var brandListParams = '<?= $signParams; ?>';
